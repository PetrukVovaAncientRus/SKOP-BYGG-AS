...
export default function ServiceCard({
  ...
  const content = (
    <div className="bg-white border-2 border-gray-100 p-8 rounded-xl shadow-sm hover:shadow-xl hover:border-brand-orange transition-all duration-300 relative overflow-hidden group h-full">
  ...
  return (
    <Link href={href} className="block h-full" aria-label={`Les mer om ${title}`}>
      {content}
    </Link>
  );
}
